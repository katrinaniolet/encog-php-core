 ...
 * Matrix math is very important to
...
class Matrix {
	 ...
	 * This will modify the matrix to
	...
	public function equals( Matrix $theMatrix, $precision = \Encog\DEFAULT_PRECISION ) {
		...
		$actualPrecision = intval( pow( \Encog\DEFAULT_PRECISION, $precision ) );
		
		$data = $theMatrix->getData();
		
		for( $r = 0; $r < $this->getRows(); ++$r ) {
			for( $c = 0; $c < $this->getCols(); ++$c ) {
				if( intval( $this->matrix[$r][$c] * $actualPrecision ) != intval( $data[$r][$c] * $actualPrecision ) ) {
					return false;
				}
			}
		}
	 ...
	 * A vector is has either a single
	 ...
	 * This is the number of elements it would take
	...
	private function validate( $row, $col ) {
		if( ($row >= $this->getRows()) || ($row < 0) ) {
			$str = "The row:" . $row . " is out of range:" . $this->getRows();
			throw new MatrixError( $str );
		}
		
		if( ($col >= $this->getCols()) || ($col < 0) ) {
			$str = "The col:" . $col . " is out of range:" . $this->getCols();
			throw new MatrixError( $str );
		}
	}
